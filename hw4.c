...
void * is_pointer(void * ptr) {
  ...
  else
  {
    ...
    void * p = mem_heap_lo();
    while(chunk_size(p)){
      if(in_use(p) && ptr < p + chunk_size(p) && p <= ptr){
        return p;
      }
      p = next_chunk(p);
    }
...
void walk_region_and_mark(void* start, void* end) {
  ...
  void *p = start;
  while(p < end - 8){
    void *_p = (void*)*(size_t*)p;
    void *h = is_pointer(_p);
    if(h  == NULL){
      p+=8;
      continue;
    }
    ...
    {
      mark((unsigned int *)h);
      //recusive call walk and passing start and end
      walk_region_and_mark(h+4, h + chunk_size(h));
    }
    
    // if(!is_marked(p)){
    //   is_marked(p) == 1;
    // }
    // mark(p); //mark block and mark everything inside block
    // size_t *next = next_chunk(p);
